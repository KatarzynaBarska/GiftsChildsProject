...
export class ChildRecord  implements ChildEntity {
    ...
    //     const [results]: [] | any = await pool.execute("SELECT * FROM `children` ORDER BY `name` ASC"); //dopisałam any
    //     return results.map((obj: ChildRecord) => new ChildRecord(obj));  // dopisałam obj: ChildRecord
    // }

    static async listAll(): Promise<ChildRecord[]> {
        const [result] = await pool.execute("SELECT * FROM `children` ORDER BY `name` ASC") as ChildRecordResults;
